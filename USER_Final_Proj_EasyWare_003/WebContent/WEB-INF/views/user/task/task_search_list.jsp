...
<c:if test="${pagingData !=null && pagingData.page_sno > 0}">
	<div align="center">
		<ul class="pagination">
			<li><a href="<c:url value='/task/search_task?currentpage=${pagingData.prev_pageno}
			&start_time=${svo.start_time}&end_time=${svo.end_time}&searchType=${svo.searchType}&searchValue=${svo.searchValue}'/>" >«</a></li>
				<c:forEach var="iter" begin="${pagingData.page_sno}" end="${pagingData.page_eno}">
				<li id="pagination"><a href="<c:url value='/task/search_task?currentpage=${iter}
			&start_time=${svo.start_time}&end_time=${svo.end_time}&searchType=${svo.searchType}&searchValue=${svo.searchValue}' />" ><b>${iter}</b></a></li>
				</c:forEach>
				<li><a href="<c:url value='/task/search_task?currentpage=${pagingData.next_pageno}
			&start_time=${svo.start_time}&end_time=${svo.end_time}&searchType=${svo.searchType}&searchValue=${svo.searchValue}'/>">»</a></li>
			</ul>
	</div>
</c:if>
