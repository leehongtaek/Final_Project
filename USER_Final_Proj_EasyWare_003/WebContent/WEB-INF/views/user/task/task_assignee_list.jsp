...
<c:if test="${pagingData !=null && pagingData.page_sno > 0}">
	<div align="center">
		<ul class="pagination">
			<li><a
				href="<c:url value='/task/assignee_list?currentpage=${pagingData.prev_pageno}'/>">«</a></li>
			<c:forEach var="iter" begin="${pagingData.page_sno}"
				end="${pagingData.page_eno}">
				<li id="pagination"><a
					href="<c:url value='/task/assignee_list?currentpage=${iter}' />"><b>${iter}</b></a></li>
			</c:forEach>
			<li><a
				href="<c:url value='/task/assignee_list?currentpage=${pagingData.next_pageno}'/>">»</a></li>
		</ul>
	</div>
</c:if>
<!-- /.PAGING END -->
