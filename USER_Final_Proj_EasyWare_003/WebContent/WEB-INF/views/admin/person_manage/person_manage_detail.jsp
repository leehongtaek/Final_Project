...
<div class="col-sm-12">
		<div class="panel panel-info">
			...
			<div class="panel-body">
				<form class="form-horizontal" action="<c:url value="/personManager/update" />" method="post" role="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							...
							<select class="form-control" name="dept_num" >
								...
								<c:if test="${row.DEPT_NUM != vo.dept_num}">
								<option value="${row.DEPT_NUM}" >${row.DEPT_NAME}</option>
								</c:if>
								<c:if test="${row.DEPT_NUM == vo.dept_num}">
								<option value="${row.DEPT_NUM}" selected >${row.DEPT_NAME}</option>
								</c:if>
								</c:forEach>
							</select>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							...
							<select class="form-control" name="posit_num">
								<option>-- 선택 --</option>
								<c:forEach items="${posit}" var="row">
								<c:if test="${row.POSIT_NUM != vo.posit_num}">
								<option value="${row.POSIT_NUM}" >${row.POSIT_NAME}</option>
								</c:if>
								<c:if test="${row.POSIT_NUM == vo.posit_num}">
								<option value="${row.POSIT_NUM}" selected >${row.POSIT_NAME}</option>
								</c:if>
								</c:forEach>
							</select>
						</div>
					...
					<div class="form-group">
						...
						<!-- 인사부가 아니면 수정은 불가 조회는 가능 -->
						<c:if test="${sessionScope.dept_num != 2 }">      
							<button type="submit" class="btn btn-primary btn-block" disabled="disabled">UPDATE</button>
						</c:if>
						</div>
						<div class="col-sm-3">
